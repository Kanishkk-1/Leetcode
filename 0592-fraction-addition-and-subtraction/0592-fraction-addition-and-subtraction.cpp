    ...
    string fractionAddition(string s) {
        ...
        while (i < n) {
            if (s[i] == '/') {
                ...
                // Check if the denominator is negative (which should not occur
                // in a valid fraction)
                if (s[j] == '-') {
                    isneg = true;
                }
                ...
                // Traverse forward to find the denominator after the '/'
                // character
                while ('0' <= s[j] && s[j] <= '9') {
                    int num = s[j] - '0';
                    res = res * 10 + num;
        ...
        // Calculate the resulting numerator by summing up all numerators after
        // adjusting them to the common denominator
        int numRes = 0;
        for (int i = 0; i < num.size(); i++) {
            numRes = numRes + (lcm / den[i]) * (num[i]);
        }

        // Find the greatest common divisor (GCD) of the resulting numerator and
        // the LCM (which is now the denominator)
        ...
        // Return the resulting fraction as a string in the form
        // "numerator/denominator"
        return std::to_string(numRes) + "/" + std::to_string(lcm);
    }

private:
    // Function to calculate the greatest common divisor (GCD) using the
    // Euclidean algorithm
    int gcd(int a, int b) {
        while (b != 0) {
            int temp = b;
